...
You can obtain, and renew, a web certificate enabling the ACME protocol for your web server and communicating with Let's Encrypt to generate the needed certificates. This is not difficult to do and may languages such as Rust, Node.js, or Go have this functionality accessible with the simple inclusion of an additional library.
...
⚠ Note that this is the only modification that you should probably manually make to your web server. Everything else will be done with automated continuous deployment processes.

1. One your console program.
...
1. Edit the Caddy's configuration (`Caddyfile`) found in the ubuntu user's home directory. Note that since this file is owned by the root user you need to use `sudo` to elevate your user to have the rights to change the file.

   ```sh
   ➜  sudo vi Caddyfile
   ```

1. Modify the Caddy rule that is specifying that any request for port 80 (HTTP) should serve up static files, to instead specify that any request to your domain name should serve up static files. By not specifying a port the rule will serve up files using port 443 (HTTPS), and any request to port 80 will automatically redirect the browser to prot 443.
...
1. Restart Caddy so that your changes will take effect.
